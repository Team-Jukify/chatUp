...
export function loadChats(filterBy) {
  return async dispatch => {
    ...
    const action = { type: 'SET_BOARDS', chats }
    dispatch(action)
  }
}
export function getChatById(chatId) {
  return async dispatch => {
    ...
    await dispatch({ type: 'SET_BOARD', chat })
    return chat
  }
}
export function saveChat(chat) {
  return async dispatch => {
    ...
    if (!chat._id) dispatch({ type: 'ADD_BOARD', chat: updatedChat })
    else dispatch({ type: 'UPDATE_BOARD', updatedChat })
    return updatedChat;
  }
}
...
export function removeChat(chatId) {
  return async dispatch => {
    ...
    if (!res) return
    dispatch({ type: 'REMOVE_BOARD', chatId })
    return res.msg;
  }
}
